document.addEventListener("DOMContentLoaded", () => {
  ...
  const groupId = urlParams.get("group"); // target group chat
  ...
  messageForm.addEventListener("submit", async (e) => {
    ...
    appendMessage({ senderId: myUserId, senderName: "You", text: content, timestamp: Date.now() });
    messageInput.value = "";
  });

  // Typing indicator
  messageInput.addEventListener("input", () => {
    socket.emit("typing", { groupId, senderId: myUserId });
    clearTimeout(typingTimeout);
    typingTimeout = setTimeout(() => {
      socket.emit("typing", { groupId, senderId: null });
    }, 2000);
  });

  // Listen for incoming group messages
  socket.on("group-message", (msg) => {
    if (msg.group === groupId && msg.senderId !== myUserId) {
      appendMessage(msg);
    }
  });
  ...
  async function handleUpload(files) {
    ...
    const res = await fetch(`/api/messages/group/upload/${groupId}`, {
      method: "POST",
