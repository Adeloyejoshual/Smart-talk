 ...
 * - professional header with avatar/name -> view profile
 * - voice/video call buttons in header (navigate to /voicecall/:uid and /videocall/:uid)
 * - 3-dot menu with 6 primary items and "More" to show rest
 * - fetches chat partner profile automatically
 * - retains existing chat features (messages, upload, reactions, recording, etc.)
 */
...
export default function ChatConversationPage() {
    ...
    const loadMeta = async () => {
      try {
        ...
        if (cSnap.exists()) {
          ...
          // detect friend id (other participant)
          const friendId = data.participants?.find((p) => p !== myUid) || null;
          // set local flags
          ...
          if (friendId) {
            ...
            else setFriendInfo({ id: friendId }); // fallback minimal
          }
        }
        // listen for chat doc changes (blockedBy, muted etc)
  ...
  // ---------- header actions (menu implementations) ----------
  const clearChat = async () => {
    if (!confirm("Clear chat? This will attempt to delete all messages.")) return;
    try {
      const msgsRef = collection(db, "chats", chatId, "messages");
  ...
  const deleteChat = async () => {
    if (!confirm("Delete this chat completely? This cannot be undone.")) return;
    try {
      ...
      setHeaderMenuOpen(false);
      navigate("/chat"); // back to chat list
    } catch (e) {
      console.error(e);
  ...
  const toggleMute = async () => {
    try {
      ...
      if (!snap.exists()) return;
      const data = snap.data();
      const mutedFor = data.mutedFor || [];
      if (mutedFor.includes(myUid)) {
        await updateDoc(chatRef, { mutedFor: arrayRemove(myUid) });
        setMuted(false);
      } else {
        await updateDoc(chatRef, { mutedFor: arrayUnion(myUid) });
        setMuted(true);
      }
      setHeaderMenuOpen(false);
    } catch (e) {
  ...
  const toggleArchive = async () => {
    try {
      ...
      if (archivedBy.includes(myUid)) {
        await updateDoc(chatRef, { archivedBy: arrayRemove(myUid) });
        setArchived(false);
      } else {
        await updateDoc(chatRef, { archivedBy: arrayUnion(myUid) });
        setArchived(true);
      }
      setHeaderMenuOpen(false);
    } catch (e) {
  ...
  const togglePinChat = async () => {
    try {
      const chatRef = doc(db, "chats", chatId);
      if (!pinned) {
        await updateDoc(chatRef, { pinnedChatFor: arrayUnion(myUid) });
        setPinned(true);
      } else {
        await updateDoc(chatRef, { pinnedChatFor: arrayRemove(myUid) });
        setPinned(false);
      }
      setHeaderMenuOpen(false);
    } catch (e) {
  ...
  const reportChat = async () => {
    // simple placeholder — in real app you'd send report details to moderation
    alert("Reported. Thank you — moderation will review this chat.");
    setHeaderMenuOpen(false);
  };

  const searchInChat = () => {
    // placeholder UI flow: you can implement a search UI
    const term = window.prompt("Search messages in this chat (simple):");
    if (!term) return;
    const found = messages.filter((m) => (m.text || "").toLowerCase().includes(term.toLowerCase()));
    if (found.length) {
      const id = found[0].id;
      const el = document.getElementById(`msg-${id}`);
      if (el) el.scrollIntoView({ behavior: "smooth", block: "center" });
      alert(`Found ${found.length} message(s). Scrolled to the first.`);
    } else {
  ...
  const changeWallpaper = () => {
    // placeholder for wallpaper/theme setting
    alert("Wallpaper picker not implemented in this demo.");
    setHeaderMenuOpen(false);
  };
  ...
  // ---------- header click handlers for profile & calls ----------
  const openProfile = () => {
  ...
  return (
    <div style={{
      ...
      <header style={{
        ...
        <button onClick={() => navigate(-1)} style={{ fontSize: 20, background: 'transparent', border: 'none', color: '#fff', cursor: 'pointer' }}>←</button>
        ...
        <div style={{ minWidth: 0, cursor: 'pointer' }} onClick={openProfile}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
          <button title="Voice call" onClick={startVoiceCall} style={{ background: "transparent", border: "none", color: "#fff", cursor: "pointer", fontSize: 20 }}>📞</button>
          <button title="Video call" onClick={startVideoCall} style={{ background: "transparent", border: "none", color: "#fff", cursor: "pointer", fontSize: 20 }}>🎥</button>

          {/* 3-dot menu */}
          <div style={{ position: "relative" }}>
            <button onClick={() => { setHeaderMenuOpen((s) => !s); setMenuExpanded(false); }} style={{ background: "transparent", border: "none", color: "#fff", cursor: "pointer", fontSize: 20 }}>⋮</button>

            {headerMenuOpen && (
              <div style={{ position: "absolute", right: 0, top: 36, background: "#fff", color: "#000", padding: 8, borderRadius: 10, boxShadow: "0 8px 30px rgba(0,0,0,0.12)", width: 220 }}>
                {/* primary 6 items */}
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <button onClick={openProfile} style={menuBtnStyle}>View Profile</button>
                  <button onClick={toggleMute} style={menuBtnStyle}>{muted ? "Unmute Notifications" : "Mute Notifications"}</button>
                  <button onClick={toggleBlock} style={menuBtnStyle}>{blocked ? "Unblock User" : "Block User"}</button>
                  <button onClick={deleteChat} style={menuBtnStyle}>Delete Chat</button>
                  <button onClick={clearChat} style={menuBtnStyle}>Clear Messages</button>
                  <button onClick={toggleArchive} style={menuBtnStyle}>{archived ? "Unarchive Chat" : "Archive Chat"}</button>

                  {/* More toggle */}
                  <button onClick={() => setMenuExpanded((s) => !s)} style={{ ...menuBtnStyle, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    More
                    <span style={{ fontSize: 12 }}>{menuExpanded ? "▾" : "▸"}</span>
                  </button>

                  {/* expanded items */}
                  {menuExpanded && (
                    <>
                      <button onClick={reportChat} style={menuBtnStyle}>Report</button>
                      <button onClick={togglePinChat} style={menuBtnStyle}>{pinned ? "Unpin Chat" : "Pin Chat"}</button>
                      <button onClick={searchInChat} style={menuBtnStyle}>Search</button>
                      <button onClick={changeWallpaper} style={menuBtnStyle}>Wallpaper / Theme</button>
                      <div style={{ height: 6 }} />
                      <button onClick={() => setHeaderMenuOpen(false)} style={{ ...menuBtnStyle, color: "#666" }}>Close</button>
                    </>
                  )}
                </div>
              </div>
