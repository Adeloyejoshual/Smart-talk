  ...
  const startRecording = async () => {
    ...
    try {
      ...
      mr.ondataavailable = (ev) => {
        if (ev.data.size) recorderChunksRef.current.push(ev.data);
      };
      mr.onstop = async () => {
...
const handleMsgTouchMove = (ev) => {
  if (!swipeStartX.current && ev.touches?.[0]) swipeStartX.current = ev.touches[0].clientX;
};
...
useEffect(() => {
  const handleClickOutside = (event) => {
    ...
    if (menuOpenFor) {
      const menuEl = document.getElementById(`msg-menu-${menuOpenFor}`);
      if (menuEl && !menuEl.contains(event.target)) setMenuOpenFor(null);
    }
  };
...
const viewProfile = () => { navigate(`/user/${friendInfo?.id}`); setHeaderMenuOpen(false); };
const reportUser = () => { alert("Reported user."); setHeaderMenuOpen(false); };
