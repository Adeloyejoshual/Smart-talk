...
import { collection, addDoc, query, orderBy, onSnapshot, serverTimestamp, updateDoc, doc, getDoc, arrayUnion, arrayRemove, deleteDoc, limit as fsLimit, getDocs } from 'firebase/firestore';
...
export default function ChatConversationPage() {
  ...
  useEffect(() => {
    ...
    const loadMeta = async () => {
      try {
        ...
        unsubChat = onSnapshot(doc(db, 'chats', chatId), (s) => { if (s.exists()) setChatInfo((p) => ({ ...(p||{}), ...s.data() })); });
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      filtered.forEach(async (m) => { if (m.senderId !== myUid && m.status === 'sent') { try { await updateDoc(doc(db, 'chats', chatId, 'messages', m.id), { status: 'delivered' }); } catch (e) {} } });
  ...
  const onFilesSelected = (e) => {
    ...
    const newPreviews = files.map((f) => ({ url: f.type.startsWith('image/') || f.type.startsWith('video/') ? URL.createObjectURL(f) : null, type: f.type, name: f.name, file: f }));
  ...
  const sendTextMessage = async () => {
    ...
    if (selectedFiles.length > 0) {
      const filesToSend = [...selectedFiles]; setSelectedFiles([]); setPreviews([]);
      for (const file of filesToSend) {
        try {
          const placeholder = { senderId: myUid, text: '', mediaUrl: '', mediaType: file.type.split('/')[0], fileName: file.name, createdAt: serverTimestamp(), status: 'uploading', reactions: {} };
          const mRef = await addDoc(collection(db, 'chats', chatId, 'messages'), placeholder);
          const messageId = mRef.id; setUploadingIds((prev) => ({ ...prev, [messageId]: 0 }));
          // Caller should provide uploadToCloudinary; for modular demo we skip actual upload
    ...
    try {
      ...
      mr.onstop = async () => {
        ...
        try {
          ...
          const mRef = await addDoc(collection(db, 'chats', chatId, 'messages'), placeholder);
          const messageId = mRef.id; setUploadingIds((p) => ({ ...p, [messageId]: 0 }));
          await updateDoc(doc(db, 'chats', chatId, 'messages', messageId), { mediaUrl: '', status: 'sent', sentAt: serverTimestamp() });
          setTimeout(() => setUploadingIds((p) => { const c = { ...p }; delete c[messageId]; return c; }), 200);
        } catch (err) { console.error(err); }
      };
  ...
  const applyReaction = async (messageId, emoji) => {
    try { const mRef = doc(db, 'chats', chatId, 'messages', messageId); const snap = await getDoc(mRef); if (!snap.exists()) return; const data = snap.data(); const existing = data.reactions?.[myUid]; if (existing === emoji) await updateDoc(mRef, { [`reactions.${myUid}`]: null }); else await updateDoc(mRef, { [`reactions.${myUid}`]: emoji }); setReactionFor(null); } catch (e) { console.error(e); }
  };

  // header actions
  const openProfile = () => { if (!friendInfo?.id) return alert('Profile not found'); navigate(`/profile/${friendInfo.id}`); setHeaderMenuOpen(false); };
  ...
  const headerPrimary = [
    ...
    { id: 'block', label: (chatInfo?.blockedBy || []).includes(myUid) ? 'Unblock' : 'Block User', action: async () => {
      const chatRef = doc(db, 'chats', chatId); const snap = await getDoc(chatRef); if (!snap.exists()) return; const data = snap.data(); const blockedBy = data.blockedBy || []; if (blockedBy.includes(myUid)) await updateDoc(chatRef, { blockedBy: arrayRemove(myUid) }); else await updateDoc(chatRef, { blockedBy: arrayUnion(myUid) }); }
    },
    { id: 'clear', label: 'Clear Chat', action: async () => { if (!confirm('Clear chat?')) return; const msgsRef = collection(db, 'chats', chatId, 'messages'); const snap = await getDocs(query(msgsRef, orderBy('createdAt', 'asc'))); for (const d of snap.docs) { try { await deleteDoc(d.ref); } catch (e) {} } } },
    { id: 'delete', label: 'Delete Chat', action: async () => { if (!confirm('Delete this chat?')) return; const msgsRef = collection(db, 'chats', chatId, 'messages'); const snap = await getDocs(query(msgsRef, orderBy('createdAt', 'asc'))); for (const d of snap.docs) { try { await deleteDoc(d.ref); } catch (e) {} } await deleteDoc(doc(db, 'chats', chatId)); navigate('/chat'); } },
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: wallpaper ? `url(${wallpaper}) center/cover no-repeat` : (isDark ? '#070707' : '#f5f5f5'), color: isDark ? '#fff' : '#000' }}>
      <Header friendInfo={friendInfo} onBack={() => navigate('/chat')} onOpenProfile={openProfile} onVoiceCall={startVoiceCall} onVideoCall={startVideoCall} onOpenMenu={() => setHeaderMenuOpen((s) => !s)} isDark={isDark} />

      <div style={{ position: 'relative' }}>{headerMenuOpen && <div style={{ position: 'absolute', right: 12, top: 0 }}><ThreeDotMenu open={headerMenuOpen} primaryActions={headerPrimary} moreActions={headerMore} onClose={() => setHeaderMenuOpen(false)} expanded={headerMenuExpanded} setExpanded={setHeaderMenuExpanded} /></div>}</div>
      ...
      <main ref={messagesRefEl} style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
        ...
        <MessageList messages={messages} myUid={myUid} friendInfo={friendInfo} isDark={isDark} onLongPress={(m,e) => { setReactionFor(m.id); }} onOpenMenu={(m) => setMenuOpenFor(m.id)} onReact={(m) => setReactionFor(m.id)} />
        <div ref={endRef} />
      </main>

      {!isAtBottom && (<button onClick={() => endRef.current?.scrollIntoView({ behavior: 'smooth' })} style={{ position: 'fixed', left: '50%', transform: 'translateX(-50%)', bottom: 120, zIndex: 80, background: '#007bff', color: '#fff', border: 'none', borderRadius: 22, width: 48, height: 48, fontSize: 22 }}>↓</button>)}
      ...
      <MessageInput text={text} setText={setText} onAttach={onFilesSelected} onSend={sendTextMessage} previews={previews} setPreviews={setPreviews} startRecording={startRecording} recording={recording} stopRecording={stopRecording} />
