...
export default function ArchivePage() {
  ...
  const handleTouchStart = (e, id) => {
    swipeRefs.current[id] = { startX: e.touches[0].clientX, moved: 0 };
  };

  const handleTouchMove = (e, id) => {
    if (!swipeRefs.current[id]) return;
    const deltaX = e.touches[0].clientX - swipeRefs.current[id].startX;
    swipeRefs.current[id].moved = deltaX;
  };

  const handleTouchEnd = (chatId) => {
    const moved = swipeRefs.current[chatId]?.moved || 0;
    if (moved < -50) {
      // swipe left detected → show options
      const confirmed = window.confirm("Unarchive this chat?");
      if (confirmed) updateDoc(doc(db, "chats", chatId), { archived: false });
    }
    swipeRefs.current[chatId] = null;
  };
