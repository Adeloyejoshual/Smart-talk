...
export default function WalletPage() {
  ...
  const handleDailyCheckin = async () => {
    if (!user || checkedInToday) return alert("✅ Already checked in today!");
    try {
      const reward = 0.25;
      const res = await axios.post("/api/wallet/daily", { uid: user.uid, amount: reward });
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-xl p-6 w-full max-w-md mb-6 flex flex-col items-center"
      >
        ...
        <div className="flex gap-4 w-full mb-4">
          <button
            ...
            className="flex-1 py-3 rounded-lg bg-blue-500 hover:bg-blue-600 font-semibold transition"
          >
            💳 Top-Up
          </button>
          <button
            onClick={() => navigate("/withdraw")}
            className="flex-1 py-3 rounded-lg bg-green-500 hover:bg-green-600 font-semibold transition"
          >
            💵 Withdraw
          </button>
        </div>
