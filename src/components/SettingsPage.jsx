...
export default function SettingsPage() {
  ...
  const handleProfileChange = async (e) => {
    ...
    try {
      ...
      setProfilePic(url);
      if (user) await updateDoc(doc(db, "users", user.uid), { profilePic: url });
    } catch (err) {
  ...
  return (
    <div style={{ padding: 20, background: isDark ? "#1c1c1c" : "#f8f8f8", color: isDark ? "#fff" : "#000", minHeight: "100vh" }}>
      <button onClick={() => navigate(-1)} style={{ position: "absolute", top: 20, left: 20, background: isDark ? "#555" : "#e0e0e0", border: "none", borderRadius: "50%", padding: 8, cursor: "pointer" }}>⬅</button>
      <h2 style={{ textAlign: "center", marginBottom: 20 }}>⚙️ Settings</h2>

      {/* Profile Picture */}
      <Section title="Profile" isDark={isDark}>
        <div onClick={handleProfileClick} style={{ width: 100, height: 100, borderRadius: "50%", overflow: "hidden", cursor: "pointer", marginBottom: 10, border: "2px solid #555" }}>
          <img src={profilePic || "/default-avatar.png"} alt="Profile" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        </div>
        <input type="file" accept="image/*" ref={profileInputRef} style={{ display: "none" }} onChange={handleProfileChange} />
        <button onClick={() => navigate("/userprofile")} style={btnStyle("#007bff")}>View Profile</button>
      </Section>
...
/* === Reusable Styles === */
const btnStyle = (bg) => ({ marginRight: 8, marginTop: 8, padding: "10px 15px", background: bg, color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: "bold" });
...
const previewBox = { width: "100%", height: 150, borderRadius: 10, border: "2px solid #555", marginTop: 15, display: "flex", justifyContent: "center", alignItems: "center", backgroundSize: "cover", backgroundPosition: "center", cursor: "pointer" };